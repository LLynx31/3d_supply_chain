...
export default function PageCategorie({ params }) {
  ...
  const [dataProductSousCategorie, setDataProductSousCategorie] =
    useState(null);

  useEffect(() => {
    fetch(
      "https://api.3dsupplychains.com/api/categories?page=1&code=" + params.code
    )
      ...
      .then((responseParse) => {
        ...
        fetch(
          "https://api.3dsupplychains.com/api/produits?page=1&sousCategorie=" +
            responseParse["hydra:member"][0].sousCategories[0]["@id"]
        )
          .then((response) => response.json())
          .then((responseParse) =>
            setDataProductSousCategorie(responseParse["hydra:member"])
          )
  ...
  if (dataSousCategorie && dataProductSousCategorie) {
    const listDataProduct = dataProductSousCategorie.map((product) => (
      <Article
      ...
      >
        ...
        <div className="px-3 sm:px-8 pt-5">
          ...
          <SousCategories
            ...
            setDataProduct={setDataProductSousCategorie}
            code={params.code}
            categorie={nameCetgorie}
          ></SousCategories>
