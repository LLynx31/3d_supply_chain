  ...
  return( 
    <motion.div initial={{opacity: 0, y:50}} animate={{opacity:1, y:0}} transition={{duration:0.3}} className="flex mt-20 justify-center h-screen ">
      ...
      <h1 className="text-lg text-center font-bold">Confirmer votre mail.</h1>
        <p className="text-base text-center font-light">Renseignez votre email et le code reçue.</p>
        <div className="w-full mt-3.5">
            {isValidCode && <div className="text-base text-center text-green-600">Votre inscription a été effectuer avec succès. <Link href={"/"} className="underline ">Cliquez ici pour vous connecter avec vos identifiants.</Link></div>}
            {isErrorCode && <div className="text-base text-center text-rouge">Code incorrect</div>}
            <div className="text-base my-2 text-blue-600">
              Un mail contenant un code de confirmation a été envoyé a l'adresse indiquée.
            </div>
            <div className="text-base">Entrez votre email.</div>
            <input className="w-full border border-gray-300 px-1 py-2 mb-2"  onChange={(e)=> setData({...isData, email:e.target.value})} type="email" placeholder="example@gmail.com"></input>
            <div className="text-base">Entrez le code que vous avez reçu.</div>
            <input className="w-full border border-gray-300 px-1 py-2" onChange={(e)=> setData({...isData, code:e.target.value})} type="text" placeholder="code"></input>
